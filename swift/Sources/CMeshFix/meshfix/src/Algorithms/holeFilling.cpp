...
int Basic_TMesh::refineSelectedHolePatches(Triangle *t0)
{
 ...
 // PATCHED (stlrepair): leftover debug print; it wrote to stdout on every
 // hole fill, which corrupts machine-readable output. Guarded, not removed.
 if (!TMesh::quiet) printf("%d\n",reg.numels());
